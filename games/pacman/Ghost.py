...
class Ghost(Character):
    ...
    def movement_logic(self): # called every frame
        ...
        # CHALLENGE 1
        if self.hunt_counter < 5:
            if elapsed_time > 7 and self.cur_mode == "scatter":
                self.change_mode("hunt")
                self.hunt_counter += 1
                self.start_time = time.time()
            if elapsed_time > 20 and self.cur_mode == "hunt":
    ...
    def make_scared(self):
        ...
        self.start_time = time.time()
        self.change_direction(get_opposite_dir(self.direction))  # CHALLENGE 2
    ...
    def get_new_direction_for_intersection(self, possible_directions):
        ...
        if self.cur_mode == "scatter":
            ...
            # CHALLENGE 3
                ...
                distance_to_pacman = math.dist((self.game.pacman.x, self.game.pacman.y), (self.x, self.y))
                if distance_to_pacman > TILE_SIZE * 8:
                    target_coords = self.game.pacman.x, self.game.pacman.y
                    return self.get_direction_closest_to_target(directions_without_walls, target_coords)
                else:
                    return self.get_direction_closest_to_target(directions_without_walls, self.target_coords)
        ...
        elif self.cur_mode == "dead":  # CHALLENGE 4
